...
int hmac(uint8_t * out, int out_size, uint8_t * input, int in_size, uint8_t * key, int key_size)
{
	...
	{
		printf("Output string not big enough to get the hash value");
		return 1;
	}
	int BLOCK_SIZE = 0;
	...
	{
		printf("Problem in the inner hash");
		return 1;
	}
	...
	{
		printf("Problem with the outer hash");
		return 1;
	}

	if (error_ind > 0)
	{
		printf("Error while processing the concatenations");
		return 1;
	}
...
int hash_func(uint8_t * hash, uint8_t * msg, int length)
{
	switch (HASH_LEN)
	{
		...
		case 64:
			...
			break;
		default:
			printf("No hash function for this output size");
			return 1;
	};
	return 0;
}
